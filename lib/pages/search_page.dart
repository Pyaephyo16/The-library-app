                      ...
                      separatorBuilder: (BuildContext context,int index) => Divider(),
                          ...
                          itemBuilder: (BuildContext context,int index){
                              return BookNameAndInfoView(
                                onClick: (){
                                  SearchPageBloc bloc = Provider.of(context,listen: false);
                                  bloc.saveBookToCarousel(filterList[index]).then((value) => 
                                  navigateToNextScreen(context, BookDetailPage(
                                    book: filterList[index],
                                  )));
                                },
                                isSheet: false,
             ...
             builder: (context,data,child) =>
                            ListView.builder(
                             ...
                             return BookListTileAndSerialsView(
                               ...
                               onClick: (indexInner){
                                SearchPageBloc bloc = Provider.of(context,listen: false);
                                bloc.saveBookToCarousel(data[data.keys.elementAt(index)]?[indexInner] ?? BookVO.empty()).then((value){
                               
                                  navigateToNextScreen(context, BookDetailPage(
                                  book: data[data.keys.elementAt(index)]?[indexInner] ?? BookVO.empty(),
                                ));
                                }  
                                );
                               },
